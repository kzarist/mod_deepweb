...
local onion_proxy_ip = module:get_option_string("onion_socks5_host", "127.0.0.1");
local onion_proxy_port = module:get_option_number("onion_socks5_port", 9050);
local i2p_proxy_ip = module:get_option_string("i2p_socks5_host", "127.0.0.1");
local i2p_proxy_port = module:get_option_number("i2p_socks5_port", 4447);
local forbid_else = module:get_option_boolean("deepweb_only", false);
local torify_all = module:get_option_boolean("deepweb_tor_all", false);
local onion_map = module:get_option("onion_map", {});
...
local onion_listener = { default_port = onion_proxy_port, default_mode = "*a", default_interface = "*" };
local i2p_listener = { default_port = i2p_proxy_port, default_mode = "*a", default_interface = "*" };


local function onion_connect_sent(conn, data)
...
local function i2p_connect_sent(conn, data)
...
local function onion_handshake_sent(conn, data)
	...
	session.socks5_handler = onion_connect_sent;
end

local function i2p_handshake_sent(conn, data)
	...
	session.socks5_handler = i2p_connect_sent;
end

function onion_listener.onconnect(conn)
	...
	sessions[conn].socks5_handler = onion_handshake_sent;
end

function i2p_listener.onconnect(conn)
	module:log("debug", "Connected to SOCKS5 proxy, sending SOCKS5 handshake.");

	-- Socks version 5, 1 method, no auth
	conn:write(c(5) .. c(1) .. c(0));

	sessions[conn].socks5_handler = i2p_handshake_sent;
end

function onion_listener.register_outgoing(conn, session)
	session.direction = "outgoing";
	sessions[conn] = session;
end

function i2p_listener.register_outgoing(conn, session)
	session.direction = "outgoing";
	sessions[conn] = session;
end

function onion_listener.ondisconnect(conn, err)
	sessions[conn]  = nil;
end

function i2p_listener.ondisconnect(conn, err)
	sessions[conn]  = nil;
end

function onion_listener.onincoming(conn, data)
	local session = sessions[conn];
...
function i2p_listener.onincoming(conn, data)
	local session = sessions[conn];
...
local function connect_onion(host_session, connect_host, connect_port)

	module:log("debug", "Connecting to " .. connect_host .. ":" .. connect_port);

	-- this is not necessarily the same as .to_host (it can be that this is from the onion_map)
	host_session.socks5_to = connect_host;
	host_session.socks5_port = connect_port;

	local conn = addclient(onion_proxy_ip, onion_proxy_port, onion_listener, "*a");

	onion_listener.register_outgoing(conn, host_session);

	host_session.conn = conn;
end

local function connect_i2p(host_session, connect_host, connect_port)
	...
	local conn = addclient(i2p_proxy_ip, i2p_proxy_port, i2p_listener, "*a");

	i2p_listener.register_outgoing(conn, host_session);
...
-- Try to intercept anything to *.i2p or *.onion
local function route_to_deepweb(event)
	...
	if not is_onion or is_i2p then
	   if onion_map[to_host] then
		 ...
		 onion_host = onion_map[to_host].host;
		 onion_port = onion_map[to_host].port;
		 is_onion = true;
	      end
	   elseif i2p_map[to_host] then
	   ...
	   elseif forbid_else then
	      module:log("debug", event.to_host .. " is not an deepweb. Blocking it.");
	      return false;
	...
	module:log("debug", "Deepweb routing something to ".. to_host);
...
module:log("debug", "Deepweb ready and loaded");

module:hook("route/remote", route_to_deepweb, 200);
